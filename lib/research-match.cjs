...
function safeUrl(value) {
  try {
    ...
    // Search citations add attribution parameters absent from the JSON evidence URL.
    // Remove only known tracking keys; keep content-identifying query parameters.
    for (const name of [...u.searchParams.keys()]) {
      if (/^utm_/i.test(name) || /^(gclid|fbclid)$/i.test(name)) u.searchParams.delete(name);
    }
    u.hash = '';
    return u.href;
  } catch { return ''; }
...
function unpack(response) {
  ...
  let data;
  try { if (blocks.length > 1) throw new Error('Multiple data blocks'); data = JSON.parse(clean); } catch {
    const error = new Error('Research returned an unreadable result. Please try again.');
    // Only the preview API exposes this travel-answer excerpt, not provider headers/config.
    error.researchDiagnostic = { stage: 'answer_format', jsonBlocks: blocks.length,
      length: text.length, beginning: text.slice(0, 1600), ending: text.slice(-1600) };
    throw error;
  }
...
function validateProfile({ data, sources }) {
  ...
  if (defining.length < 2 || !defining.some(f => f.kind === 'scale') || domains.size < 2) {
    const error = new Error('Not enough reliable information about that place yet. Try a more specific place name.');
    // Restricted to public citation URLs and enum/count metadata, never raw provider output.
    error.researchDiagnostic = { stage: 'source_profile', definingCount: defining.length,
      hasScale: defining.some(f => f.kind === 'scale'), domainCount: domains.size,
      citedSources: [...sources.keys()].slice(0, 20),
      featureEvidence: data.features.slice(0, 10).filter(f => f && KINDS.has(f.kind)).map(f => ({
        kind: f.kind, defining: f.importance === 'defining',
        proposedUrls: (Array.isArray(f.evidenceUrls) ? f.evidenceUrls : []).map(safeUrl).filter(Boolean).slice(0, 6),
        acceptedUrls: evidence(f.evidenceUrls, sources) })) };
    throw error;
  }
...
function rankCandidates(profile, { data, sources }, diagnostics = []) {
  ...
  for (const c of data.candidates.slice(0, 12)) {
    ...
    const diagnostic = { match, rejected: '', comparisons: [] };
    diagnostics.push(diagnostic);
    if (!match || !region || seen.has(identity) || c.inScope !== true || !PLACE_TYPES.has(c.placeType)) { diagnostic.rejected = 'identity, scope or duplicate'; continue; }
    ...
    if (['hamlet', 'village', 'small_town', 'rural_area'].includes(profile.placeType) &&
        ['urban_neighborhood', 'city', 'suburb'].includes(c.placeType)) { diagnostic.rejected = 'urban settlement mismatch'; continue; }
    ...
    const comparisons = defining.map(f => {
      ...
      diagnostic.comparisons.push({ feature: f.kind, fit: a?.fit || 'missing', reason: tidy(a?.reason), acceptedUrls: urls,
        proposedUrls: (Array.isArray(a?.evidenceUrls) ? a.evidenceUrls : []).map(safeUrl).filter(Boolean).slice(0, 6) });
    ...
    if (comparisons.some(a => !a || a.fit === 'mismatch' || a.fit === 'unknown')) { diagnostic.rejected = 'defining feature mismatch or missing evidence'; continue; }
    const urls = evidence(c.evidenceUrls, sources);
    if (!urls.length || !tidy(c.blurb) || !tidy(c.caveat)) { diagnostic.rejected = 'missing candidate evidence or explanation'; continue; }
...
function createMatcher({ fetchImpl = fetch, env = process.env, cache = createCache() } = {}) {
  ...
  return async function match(place, region, priorities = []) {
    ...
    return cache(`${VERSION}|${model}|matches|${key(place)}|${profile.researchedAt}|${key(region)}|${[...priorities].sort().join('|')}`, 21600000,
      async () => {
        const researched = await research(candidatePrompt(profile, region, priorities), 65000);
        const diagnostics = [];
        const results = rankCandidates(profile, researched, diagnostics);
        return { results, ...(env.VERCEL_ENV === 'preview' ? { researchDiagnostic: {
          stage: 'candidate_ranking', candidates: diagnostics, citedSources: [...researched.sources.keys()] } } : {}),
          research: { profile, researchedAt: new Date().toISOString(),
