...
test('photos require matching place and geography and include credits',()=>{
 ...
 const unknown=context.card({...item,region:'Oregon, USA'},0);assert.ok(unknown.includes('<img'));assert.ok(unknown.includes('Illustrative photo'));assert.ok(!unknown.includes('Astoria_Park'));
});
test('theme follows destination attributes, not the origin in comparison text',()=>{
 assert.equal(context.photoTheme({tags:['river town']}),'river');
 assert.equal(context.photoTheme({tags:['coastal']}),'coast');
 assert.equal(context.photoTheme({tags:['woodlands']}),'trees');
 assert.equal(context.photoTheme({blurb:'Unlike the river town you started with',tags:['food']}),'coffee');
 assert.ok(context.card({match:'Narrowsburg',tags:['river town']},0).includes('/photos/river.jpg'));
});
